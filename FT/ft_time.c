...
static double evolve(dcomplex u0[NZ][NY][NX], dcomplex u1[NZ][NY][NX],
...
static double compute_indexmap(int indexmap[NZ][NY][NX], int d[3]);
static void print_timers(void);
static void fft(int dir, dcomplex x1[NZ][NY][NX], dcomplex x2[NZ][NY][NX], double* time_cffts1, double* time_cffts2, double* time_cffts3);
static double cffts1(int is, int d[3], dcomplex x[NZ][NY][NX],
		   dcomplex xout[NZ][NY][NX],
		   dcomplex y0[NX][FFTBLOCKPAD],
		   dcomplex y1[NX][FFTBLOCKPAD]);
static double cffts2(int is, int d[3], dcomplex x[NZ][NY][NX],
		   dcomplex xout[NZ][NY][NX],
		   dcomplex y0[NX][FFTBLOCKPAD],
		   dcomplex y1[NX][FFTBLOCKPAD]);
static double cffts3(int is, int d[3], dcomplex x[NZ][NY][NX],
		   dcomplex xout[NZ][NY][NX],
		   dcomplex y0[NX][FFTBLOCKPAD],
		   dcomplex y1[NX][FFTBLOCKPAD]);
...
static double checksum(int i, dcomplex u1[NZ][NY][NX], int d[3]);
...
c-------------------------------------------------------------------*/
	...
	double time_cffts1, time_cffts2, time_cffts3;
	time_cffts1 = 0;
	time_cffts2 = 0;
	time_cffts3 = 0;

    fft(1, u1, u0, &time_cffts1, &time_cffts2, &time_cffts3);
...
c-------------------------------------------------------------------*/
    ...
    double time_compute_indexmap;
	time_compute_indexmap = compute_indexmap(indexmap, dims[2]);
	...
	double time_evolve, time_checksum;
	time_evolve = 0;
	time_checksum = 0;
    ...
    for (iter = 1; iter <= niter; iter++) {
	    ...
	    time_evolve += evolve(u0, u1, iter, indexmap, dims[0]);	
	      ...
	      timer_start(T_FFT);
            ...
            fft(-1, u1, u2, &time_cffts1, &time_cffts2, &time_cffts3);
	      ...
	      timer_start(T_CHECKSUM);
            ...
            time_checksum += checksum(iter, u2, dims[0]);
	...
	printf("\nTime Taken by parallel loops:\n");
	printf("Time taken by compute index: %f\n", time_compute_indexmap);
	printf("Time taken by evolve: %f\n", time_evolve);
	printf("Time taken by checksum: %f\n", time_checksum);
	printf("Time taken by cffts1: %f\n", time_cffts1);
	printf("Time taken by cffts2: %f\n", time_cffts2);
	printf("Time taken by cffts3: %f\n", time_cffts3);
...
c-------------------------------------------------------------------*/
    ...
    if (dir == 1) {
        *time_cffts1 += cffts1(1, dims[0], x1, x1, y0, y1);	/* x1 -> x1 */
        *time_cffts2 += cffts2(1, dims[1], x1, x1, y0, y1);	/* x1 -> x1 */
        *time_cffts3 += cffts3(1, dims[2], x1, x2, y0, y1);	/* x1 -> x2 */
    } else {
	*time_cffts3 += cffts3(-1, dims[2], x1, x1, y0, y1);	/* x1 -> x1 */
    *time_cffts2 += cffts2(-1, dims[1], x1, x1, y0, y1);	/* x1 -> x1 */
    *time_cffts1 += cffts1(-1, dims[0], x1, x2, y0, y1);	/* x1 -> x2 */
    }
}
