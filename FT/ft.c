...
#pragma omp parallel for default(shared) private(i,j,k) schedule(static, 2) 
    for (k = 0; k < d[2]; k++) {
	for (j = 0; j < d[1]; j++) {
            for (i = 0; i < d[0]; i++) {
...
#pragma omp parallel for default(shared) private(i,j,k,ii,ii2,jj,ij2,kk) schedule(static, 2) 
